...
from models.archive import PPOActor, PPOCritic
...
class Trainer():
    def __init__(self, device, actor, actor_optimizer, critic, critic_optimizer, gamma=0.9, epsilon=0.2, gae_lambda=0.90 ,num_update_iters=10, num_episodes=500, num_envs=10):
        self.device = device
        ...
        self.env = gym.vector.AsyncVectorEnv([lambda: gym.make("Pendulum-v1") for _ in range(self.num_envs)])
    ...
    def finish_episode(self):
        ...
        advantages, returns = self.compute_gae(rewards, state_values, normalize=False)

        for epoch in range(self.num_update_iters):
            new_log_probs = self.actor.get_logp_pi_from_u(states, pre_squashed_actions)
    ...
    def train(self):
        for i_episode in range(self.num_episodes):
            ...
            for t in count():
                states = torch.from_numpy(states).float().to(self.device)  # Shape: (n, 3)
                t_feature = torch.full((states.shape[0], 1), t, device=self.device)  # Shape: (n, 1)
                states = torch.cat((states, t_feature), dim=1)  # Shape: (n, 4)

                with torch.no_grad():
                    action, log_prob, pre_squashed_action = actor.get_action(states)
                
                new_states, rewards, terminated, truncated, info = self.env.step(action.to('cpu').numpy())

                self.transition_buffer.save_transitions(pre_squashed_action.detach(), log_prob.detach(), states, torch.from_numpy(rewards).to(self.device).unsqueeze(-1))
            ...
            critic_loss = self.finish_episode()
...
if __name__ == "__main__":
    ...
    actor = PPOActor(num_features=4).to(device)
    critic = PPOCritic(num_features=4).to(device)
    actor_optimizer = optim.Adam(actor.parameters(), lr=1e-3)
    critic_optimizer = optim.Adam(critic.parameters(), lr=1e-3)

    trainer = Trainer(device, actor, actor_optimizer, critic, critic_optimizer, epsilon=0.1, num_episodes=2000, num_envs=10)
    trainer.train()